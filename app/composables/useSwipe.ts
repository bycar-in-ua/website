...
export function useSwipe(
  ...
  const {
    threshold = 50, onSwipeEnd, onSwipeStart,
  } = options;

  const coordsStart = reactive<Position>({
    x: 0,
    y: 0,
  });
  const coordsEnd = reactive<Position>({
    x: 0,
    y: 0,
  });
  ...
  const listenerOptions: { passive?: boolean;
    capture?: boolean; } = { passive: true };
